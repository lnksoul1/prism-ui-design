...
| `design_review_and_improve` | 实时 | 一键评审循环：评分 → 自动修复 → 复评 + 无障碍审计 |
...
- 自动排列：选中多个形状一键排成整齐纵列
- 组件库拖拽直落画布：左侧设计库的组件直接拖进画布，在落点生成对应 UI 块并自动保存
...
- 内置指令引擎：改主色/背景色（色名或 #hex）、深色/浅色模式、14 种风格预设、5 种页面模板、常用组件、撤销、清空——**无需外接 Agent 也能立即执行并显示结果**；未匹配指令才排队给 Agent
...
# 重新打包 tldraw 画布 bundle（client/vendor/）
