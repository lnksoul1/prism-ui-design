...
> - ✅ **方案A 第二轮（画布 v2：真实感 + AI 绘制 + 自动排列）**：
>   - 令牌驱动自定义形状：组件不再是灰块，而是按设计令牌着色的"UI 块"（按钮主色、卡片表面色、Hero/CTA 主色浅底、导航/页脚区分、文字形状无底），令牌变更实时重涂已有组件形状（位置/大小不变）
>   - `design_draw_canvas`（+ `POST /api/canvas/draw`）：AI 以 rect/text/arrow/image/prism 简单指令排队绘制，画布打开时经 WS 实时出现（已应用后自动清队列并保存）
>   - 画布"自动排列"：选中形状一键排成整齐纵列（tldraw 原生无此能力）
